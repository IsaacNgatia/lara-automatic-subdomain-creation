<div class="box-body space-y-3">
            ...
            <tbody>
                ...
                @else
                    @foreach ($pppoeUsers as $pppoeUser)
                        <tr wire:key="{{ $pppoeUser->id }}" class="border-b border-defaultborder">
                            <td>{{ $loop->iteration + ($pppoeUsers->currentPage() - 1) * $pppoeUsers->perPage() }}</td>
                            <th scope="row">
                                <a href="{{ route('customer.view.one', $pppoeUser->id) }}"
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span>{{ $pppoeUser->official_name }}</span>
                                    </div>
                                </a>
                            </th>
                            <td>{{ $pppoeUser->mikrotik_name }}</td>
                            <td>{{ $pppoeUser->password }}</td>
                            <td>{{ $pppoeUser->reference_number }}</td>
                            <td>{{ $pppoeUser->mikrotik_identity }}</td>
                            <td>{{ $pppoeUser->profile }}</td>
                            <td>{{ $pppoeUser->phone_number }}</td>
                            <td>{{ $pppoeUser->billing_amount }}</td>
                            <td>{{ $pppoeUser->billing_cycle }}</td>
                            <td>
                                @if ($pppoeUser->status == 'active')
                                    <span class="badge bg-success/10 text-success">Active</span>
                                @elseif ($pppoeUser->status == 'suspended')
                                    <span class="badge bg-light text-dark">Suspended</span>
                                @elseif ($pppoeUser->status == 'inactive')
                                    <span class="badge bg-danger text-dark">Inactive</span>
                                @endif
                            </td>
                            <td>{{ $pppoeUser->expiry_date }}</td>
                            <td>
                                <div class="hstack gap-2 flex-wrap">
                                    <div wire:click="editPppoeUser({{ $pppoeUser->id }})"
                                    ...
                                    <div wire:click="warn({{ $pppoeUser->id }})"
                                        class="hs-tooltip ti-main-tooltip [--placement:top]">
        ...
        <div class="flex ">
            <div class="flex space-x-4 items-center mb-3">
                ...
                <select wire:model.live.debounce.200ms="perPage"
                    ...
                    @if ($totalPppoeUsers > 10)
                        <option value="10">10</option>
                    @endif

                    @if ($totalPppoeUsers > 20)
                        <option value="20">20</option>
                    @endif

                    @if ($totalPppoeUsers > 50)
                        <option value="50">50</option>
                    @endif

                    @if ($totalPppoeUsers > 100)
                        <option value="100">100</option>
                    @endif

                    <option value="{{ $totalPppoeUsers }}">
                        {{ $totalPppoeUsers <= 100 ? 'Show All' : '100+' }}
                    </option>
                </select>
            </div>
        </div>
    ...
    <x-modal maxWidth="4xl" preventModalClose=true>
        @slot('slot')
            ...
            @elseif ($deletingId)
                <livewire:admins.components.modals.delete-item :title="'Delete PPPoE User'" :message="'Are you sure you want to delete PPPoE User id ' . $deletingId" :eventToBeDispatched="'delete-pppoe-user'"
                    :cancelEvent="'cancel-delete-pppoe-user'" :list="['Wallet Transactions', 'Service Plans', 'Tickets']" lazy />
            @endif
        @endslot
    </x-modal>
</div>
