                ...
                <form wire:submit="addTemplate" class="space-y-4 mt-0">
                    <div class="grid grid-cols-12 gap-4">
                        <div class="col-span-12">
                            <label for="expenseTypeName" class="form-label">SMS Type</label>
                            <select wire:model.live="subject" id="day-to-send" class="form-select !py-[0.59rem]">
                                <option value="">Select Type</option>
                                @foreach ($subjectList as $subjectItem)
                                    <option value="{{ $subjectItem['value'] }}">{{ $subjectItem['title'] }}</option>
                                @endforeach
                            </select>

                            <!-- Display the description for the selected subject -->
                            @if ($subject)
                                @php
                                    $selectedSubject = collect($subjectList)->firstWhere('value', $subject);
                                @endphp
                                <p class="mt-2 bg-secondary/50 w-auto">Description:
                        ...
                        <div class="col-span-12">
                            ...
                            <div class="ti-btn-list space-x-1 space-y-1 rtl:space-x-reverse flex-wrap mb-2">
                                @if ($subject && $subject !== 'hsp_message')
                                    <button type="button"
                                    ...
                                    <button type="button"
                                        class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                        value="{user_url}"
                                        onclick="insertAtCursor('{user_url}', 'add-sms-template')">User
                                        Account
                                        URL</button>
                                @elseif ($subject === 'hsp_message')
                                    <button type="button"
                                        class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                        value="{first_name}"
                                    ...
                                    <button type="button"
                                        class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                        value="{hsp_amount_received}"
                                        onclick="insertAtCursor('{hsp_amount_received}', 'add-sms-template')">Amount
                                        Paid
                                    </button>
                                    <button type="button"
                                        class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                        value="{hsp_datalimit}"
                                        onclick="insertAtCursor('{hsp_datalimit}', 'add-sms-template')">
                                        Data Limit
                                    </button>
                                    <button type="button"
                                        class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                        value="{hsp_timelimit}"
                                        onclick="insertAtCursor('{hsp_timelimit}', 'add-sms-template')">
                                        Time Limit
                                    </button>
                                    <button type="button"
                                        class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                    ...
                                    <button type="button"
                                        class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                        value="{payment_date}"
                                        onclick="insertAtCursor('{payment_date}', 'add-sms-template')">Payment
                                        Date</button>
                                @endif
                            </div>

                            <textarea wire:model="template" id="add-sms-template" class="ti-form-input" rows="8"
                    ...
                    <div class="">
                        <button wire:loading.attr="disabled" wire:target="addTemplate"
                            ...
                            <span wire:loading wire:target="addTemplate">Saving SMS Template</span>
                        </button>
                    </div>
                </form>
            </div>
