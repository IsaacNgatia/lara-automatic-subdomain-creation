                ...
                <form wire:submit="updateTemplate" class="space-y-4 mt-0">
                        ...
                        <div class="col-span-12">
                            ...
                            <div class="ti-btn-list space-x-1 space-y-1 rtl:space-x-reverse flex-wrap mb-2">
                                <button type="button"
                                    ...
                                    value="{official_name}"
                                    onclick="insertAtCursor('{official_name}', 'edit-expiry-sms-template')">Official
                                    Name</button>
                                <button type="button"
                                    class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                    value="{reference_number}"
                                    onclick="insertAtCursor('{reference_number}', 'edit-expiry-sms-template')">Reference
                                    Number</button>
                                <button type="button"
                                    class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                    value="{phone}"
                                    onclick="insertAtCursor('{phone}', 'edit-expiry-sms-template')">Mobile
                                    Number</button>
                                <button type="button"
                                    class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                    value="{bill}" onclick="insertAtCursor('{bill}', 'edit-expiry-sms-template')">Bill
                                    Amount</button>
                                <button type="button"
                                    class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                    value="{expiry_date}"
                                    onclick="insertAtCursor('{expiry_date}', 'edit-expiry-sms-template')">Expiry
                                    Date</button>
                                <button type="button"
                                    class="ti-btn-outline-secondary border border-secondary py-0.5 px-1.5 !rounded-full btn-wave text-xs md:text-sm"
                                    value="{user_url}"
                                    onclick="insertAtCursor('{user_url}', 'edit-expiry-sms-template')">User
                                    Account
                                    URL</button>
                            </div>

                            <textarea wire:model="template" id="edit-expiry-sms-template" class="ti-form-input" rows="8"
                    ...
                    <div class="">
                        <button wire:loading.attr="disabled" wire:target="updateTemplate"
                            ...
                            <span wire:loading wire:target="updateTemplate">Updating Expiry Template</span>
                        </button>
                    </div>
                </form>
            </div>
