            ...
            <div class="box-header justify-between">
                ...
                <div class="prism-toggle">
                    <button wire:click="openAddNew" type="button"
                        class="ti-btn btn-wave !py-1 !px-2 ti-btn-primary !font-medium !text-[0.75rem]">Add Expiry
                        Alert<i class="ri-add-line ms-2 inline-block align-middle"></i></button>
                </div>
            </div>
                    ...
                    <table class="table whitespace-normal table-bordered min-w-full">
                        <thead>
                            <tr class="border-b border-defaultborder">
                                ...
                                <th scope="col" class="text-start">Days Before/After</th>
                            ...
                            @else
                                @foreach ($allTemplates as $template)
                                    <tr wire:key="{{ $template->id }}" class="border-b border-defaultborder">
                                        ...
                                        <td>{{ $template->day_to_send . ' Day(s) (' . $template->before_after . ')' }}
                                        </td>
            ...
            <x-modal maxWidth="lg" preventModalClose=true>
                @slot('slot')
                    @if ($addNewIsOpen === true)
                        <livewire:admins.sms.modal.add-expiry-sms />
                    @elseif ($updatingId)
                        <livewire:admins.sms.modal.edit-expiry-template :id="$updatingId" lazy />
                    @elseif ($deletingId)
