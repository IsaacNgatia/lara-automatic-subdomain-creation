<form wire:submit="submitForm" class="p-2 md:px-5 md:py-2 space-y-4">
    ...
    <div>
        <div class="flex justify-between items-center">
            <label class="ti-form-select">Select Mikrotiks</label>
            {{-- <p class="flex gap-1"><span>Recipients: </span><span>
                    {{ $recipients }}</span> --}}
            </p>
        </div>
        <div class="relative">
            <x-multi-select model="sendToMikrotikForm.selectedMikrotiks" class="select-custom"
                placeholderValue="Type to search mikrotiks" :options="$mikrotiks" valueKey="id" displayName="name" />
            <div class="flex items-center mt-2 gap-4 ml-2">
                <div class="form-check form-check-inline">
                    <input wire:model.live="sendToAll" class="form-check-input single-user-checkbox" type="checkbox"
                        id="allMikrotiks">
                    <label class="form-check-label" for="allMikrotiks">Send to All</label>
                </div>
                <div class="form-check form-check-inline">
                    <input wire:model.live="sendToActive" class="form-check-input single-user-checkbox" type="checkbox"
                        id="mikrotiksWithActiveOnly">
                    <label class="form-check-label" for="mikrotiksWithActiveOnly">Send to Active
                        Only</label>
                </div>
                <div class="form-check form-check-inline">
                    <input wire:model.live="sendToInactive"class="form-check-input single-user-checkbox" type="checkbox"
                        id="mikrotiksWithInactiveOnly">
                    <label class="form-check-label" for="mikrotiksWithInactiveOnly">Send to Inactive
                        Only</label>
                </div>
            </div>
        </div>
        @error('sendToMikrotikForm.selectedMikrotiks')
    ...
    <div>
        ...
        <div class="ti-btn-list space-x-1 space-y-1 rtl:space-x-reverse flex-wrap mb-2">
            <button type="button"
                ...
                value="{official_name}" onclick="insertAtCursor('{official_name}', 'send-to-mikrotik-form')">Official
                Name</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{reference_number}"
                onclick="insertAtCursor('{reference_number}', 'send-to-mikrotik-form')">Reference
                Number</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{phone}" onclick="insertAtCursor('{phone}', 'send-to-mikrotik-form')">Mobile Number</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{bill}" onclick="insertAtCursor('{bill}', 'send-to-mikrotik-form')">Bill Amount</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{expiry_date}" onclick="insertAtCursor('{expiry_date}', 'send-to-mikrotik-form')">Expiry
                Date</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{user_url}" onclick="insertAtCursor('{user_url}', 'send-to-mikrotik-form')">User Account
                URL</button>
        </div>
        <textarea wire:model="sendToMikrotikForm.textMessage" id="send-to-mikrotik-form" class="ti-form-input" rows="3"
            placeholder="Type your message here"></textarea>
        @error('sendToMikrotikForm.textMessage')
            <div class="text-red-500 text-xs">{{ $message }}</div>
        @enderror
    </div>
