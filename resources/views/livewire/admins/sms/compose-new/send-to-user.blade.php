<form wire:submit="submitForm" class="p-2 md:px-5 md:py-2 space-y-4">
    ...
    <div>
        <div class="flex justify-between items-center">
            <label class="ti-form-select">Select Users</label>
            <p class="flex gap-1"><span>Recipients: </span><span>
                    {{ sizeof($this->sendToUsersForm->selectedUsers) }}</span>
            </p>
        </div>
        <!-- Select -->
        <div class="relative">
            <!-- Select -->
            <x-multi-select model="sendToUsersForm.selectedUsers" class="select-custom"
                placeholderValue="Type to search users" :options="$customers" valueKey="id" displayName="official_name" />
            <!-- End Select -->
        ...
        <!-- End Select -->
        <!-- Error message -->
        @error('sendToUsersForm.selectedUsers')
    ...
    <div>
        ...
        <div class="ti-btn-list space-x-1 space-y-1 rtl:space-x-reverse flex-wrap mb-2">
            <button type="button"
                ...
                value="{official_name}" onclick="insertAtCursor('{official_name}', 'send-to-users-form')">Official
                Name</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{reference_number}"
                onclick="insertAtCursor('{reference_number}', 'send-to-users-form')">Reference
                Number</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{phone}" onclick="insertAtCursor('{phone}', 'send-to-users-form')">Mobile Number</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{bill}" onclick="insertAtCursor('{bill}', 'send-to-users-form')">Bill Amount</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{expiry_date}" onclick="insertAtCursor('{expiry_date}', 'send-to-users-form')">Expiry
                Date</button>
            <button type="button"
                class="ti-btn-outline-secondary border border-secondary py-0.5 px-2 !rounded-full btn-wave text-xs md:text-sm"
                value="{user_url}" onclick="insertAtCursor('{user_url}', 'send-to-users-form')">User Account
                URL</button>
        </div>

        <textarea wire:model="sendToUsersForm.textMessage" id="send-to-users-form" class="ti-form-input" rows="5"
            placeholder="Type your message here"></textarea>
        @error('sendToUsersForm.textMessage')
            <div class="text-red-500 text-xs">{{ $message }}</div>
        @enderror
    </div>
